...
def validation_split(D_exp, val_fraction):
    ...
    if val_fraction > 0:
        ...
        I_train = range(n)
        I_valid = []
    print("the value of val_fraction:", val_fraction)
...
def save_config(fname):
    ...
    flagdict = FLAGS.flag_values_dict()  # Get the dictionary of flag values.
    config_str = '\n'.join(f"{k}: {v}" for k, v in sorted(flagdict.items()))
...
def load_data(fname):
    ...
    with np.load(fname) as data_in:
        data = {}
        # Directly access the keys since we know they exist
        data['x'] = data_in['x']
        data['t'] = data_in['t']
        data['yf'] = data_in['yf']

        # 'ycf' might be optional; check for its existence before loading
        if 'ycf' in data_in:
            data['ycf'] = data_in['ycf']
        else:
            data['ycf'] = None

        # Additional data elements can be added similarly
        # if 'ate' in data_in:
        #     data['ate'] = data_in['ate']
        # ... and so on for 'mu1', 'mu0', 'yadd', 'ymul'

        # Determine whether we have ground truth counterfactuals
        data['HAVE_TRUTH'] = 'ycf' in data_in

        # Set dimensions based on loaded data
        data['dim'] = data['x'].shape[1]
        data['n'] = data['x'].shape[0]
