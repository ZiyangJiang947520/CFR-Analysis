...
tf.app.flags.DEFINE_float('p_alpha', 5e-4, """Imbalance regularization param. """)
...
tf.app.flags.DEFINE_float('lrate', 0.07, """Learning rate. """)
...
tf.app.flags.DEFINE_float('wass_lambda', 1.6, """Wasserstein lambda. """)
...
tf.app.flags.DEFINE_string('outdir', 'C:\\AI_Project\\results\\example_ihdp', """Output directory. """)
...
tf.app.flags.DEFINE_integer('repetitions', 3, """Repetitions with different seed.""")
...
tf.app.flags.DEFINE_string('imb_fun', 'mmd_rbf', """Which imbalance penalty to use (mmd_lin/mmd_rbf/mmd2_lin/mmd2_rbf/lindisc/wass). """)
...
tf.app.flags.DEFINE_float('val_part', 0.4, """Validation part. """)
...
def run(outdir):
    ...
    for i_exp in range(1,n_experiments+1):
        ...
        print("I_valid shape:", np.shape(I_valid))
        print("I_valid contents:", I_valid)
        ''' Run training loop '''
